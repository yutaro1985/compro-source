...
func main() {
	...
	ans = inv[1] + 1
	// }
	ChmaxInt(&ans, inv[len(inv)-1]-inv[len(inv)-2]+1)
	for i := 0; i+2 < len(inv); i++ {
		if h[inv[i]] < h[inv[i+1]] && h[inv[i+1]] > h[inv[i+2]] {
			ChmaxInt(&ans, inv[i+2]-inv[i]+1)
			// fmt.Println("ansc", ans, i, h[inv[i+2]], h[inv[i+1]], h[inv[i]])
		}
		// fmt.Println("ans", ans, i, h[inv[i+2]], h[inv[i+1]], h[inv[i]])
	}
	// fmt.Println(inv)
	fmt.Println(ans)
}
