...
func main() {
	N, K := nextInt(), nextInt()
	var ans int
	performance := make([][]int, 1)
	performance[0] = append(performance[0], nextInt())
	for i := 1; i < N; i++ {
		A := nextInt()
		if performance[len(performance)-1][len(performance[len(performance)-1])-1] >= A {
			performance = append(performance, []int{A})
		} else {
			performance[len(performance)-1] = append(performance[len(performance)-1], A)
		}
	}
	for _, v := range performance {
		ans += MaxOf(0, len(v)-K+1)
	}
	fmt.Println(ans)
}
