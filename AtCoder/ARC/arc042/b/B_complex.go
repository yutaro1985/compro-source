...
func main() {
	P := nextComplex128()
	N := nextInt()
	V := make([]complex128, N+1)
	ans := math.MaxFloat32
	for i := 0; i < N; i++ {
		V[i] = nextComplex128()
		ans = math.Min(ans, cmplx.Abs(V[i]-P))
	}
	V[N] = V[0]
	for i := 0; i < N; i++ {
		a, b := V[i], V[i+1]
		...
		p := P - a
		a -= a
		// cmplx.Polarで返るθは-π〜π。
		R, theta := cmplx.Polar(b)
		r := cmplx.Rect(1, theta)
		p = p / r
		if real(p) >= 0 && real(p) <= R {
			ans = math.Min(ans, math.Abs(imag(p)))
		}
	}
	fmt.Println(ans)
}
