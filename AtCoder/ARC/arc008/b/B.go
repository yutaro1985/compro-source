...
func main() {
	N, M := nextInt(), nextInt()
	name, kit := nextLine(), nextLine()
	nameletters := make(map[byte]int)
	kitletters := make(map[byte]int)
	for i := 0; i < N; i++ {
		nameletters[name[i]]++
	}
	for i := 0; i < M; i++ {
	...
	var ans int
	for i := 0; i < N; i++ {
		if knum, exists := kitletters[name[i]]; exists {
			nnum := nameletters[name[i]]
			// 切り上げしないと足りなくなるケースがある
			// 切り上げの仕方はこちら
			// https://daeudaeu.com/programming/c-language/kurisuke_kiriage_shisyagonyu/
			ans = MaxOf(ans, (nnum+knum-1)/knum)
		} else {
			fmt.Println(-1)
			return
		}
