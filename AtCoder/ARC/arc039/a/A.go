...
func main() {
	...
	var ans, Am, Bm int
	if A[0] != '9' {
		Am = 900 + int(A[1]-'0')*10 + int(A[2]-'0')
	} else if A[1] != '9' {
		Am = int(A[0]-'0')*100 + 90 + int(A[2]-'0')
	} else {
		Am = int(A[0]-'0')*100 + int(A[1]-'0')*10 + 9
	}
	if B[0] != '1' {
		Bm = 100 + int(B[1]-'0')*10 + int(B[2]-'0')
	} else if B[1] != '0' {
		Bm = int(B[0]-'0')*100 + int(B[2]-'0')
	} else {
		Bm = int(B[0]-'0')*100 + int(B[1]-'0')*10
	}
	Ai, _ := strconv.Atoi(A)
	Bi, _ := strconv.Atoi(B)
	ans = MaxOf(Am-Bi, Ai-Bm)
	fmt.Println(ans)
}
