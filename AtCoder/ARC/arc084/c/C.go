...
func main() {
	...
	B := makeInts(N)
	C := makeInts(N)
	ReverseSort(sort.IntSlice(A))
	ReverseSort(sort.IntSlice(B))
	ReverseSort(sort.IntSlice(C))
	Csum := make([]int, N+1)
	for i := 0; i < N; i++ {
		idx := sort.Search(N, func(j int) bool { return B[i] >= C[j] })
		Csum[i+1] = Csum[i] + idx
	}
	var ans int
	for i := 0; i < N; i++ {
		ib := sort.Search(N, func(j int) bool { return A[i] >= B[j] })
		ans += Csum[ib]
	}
	fmt.Println(ans)
}
