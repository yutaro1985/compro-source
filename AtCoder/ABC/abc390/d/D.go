...
func main() {
	...
	assignment[0] = 1
	var dfs func(i int, currentAssignment []int, N int)
	dfs = func(i int, currentAssignment []int, N int) {
		if i == N {
			count[calcXOR(currentAssignment, A)] = true
			return
		}

		maxAssign := 0
		for j := 0; j < i; j++ {
			ChmaxInt(&maxAssign, currentAssignment[j])
		}

		for j := 1; j <= maxAssign+1; j++ {
			currentAssignment[i] = j
			dfs(i+1, currentAssignment, N)
		}
	}
...
func calcXOR(currentAssignment []int, A []int) int {
	N := len(A)
	groupSums := make([]int, N+1)
	for i := 0; i < N; i++ {
		groupSums[currentAssignment[i]] += A[i]
	}
	// groups := make(map[int]int)
	// for i := 0; i < len(currentAssignment); i++ {
	// 	groups[currentAssignment[i]] += A[i]
	// }
	res := 0
