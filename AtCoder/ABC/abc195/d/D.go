...
func main() {
	...
	Baggages := make([]Baggage, N)
	for i := 0; i < N; i++ {
		Baggages[i] = Baggage{nextInt(), nextInt()}
	}
	sort.Slice(Baggages, func(i, j int) bool {
		if Baggages[i].V == Baggages[j].V {
			return Baggages[i].W < Baggages[j].W
		}
		return Baggages[i].V > Baggages[j].V
	})
	X := make([]Box, M)
	for i := 0; i < M; i++ {
		X[i] = Box{nextInt(), i}
	...
	for ; Q > 0; Q-- {
		...
		for i := 0; i < N; i++ {
			for j := 0; j < M; j++ {
				if L <= X[j].idx && X[j].idx <= R {
					continue
				}
				if used[j] {
					continue
				}
				if !used[j] && X[j].X >= Baggages[i].W {
					ans += Baggages[i].V
					used[j] = true
					break
				}
