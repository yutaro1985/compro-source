...
func main() {
	...
	steps[0] = a
	for i := 0; ; i++ {
		if _, e := memo[b[prev]]; e {
			startindex = memo[b[prev]]
			endindex = i
			break
		}
		memo[prev] = i
		steps = append(steps, b[prev])
		prev = b[prev]
	}
	if kbig.Cmp(big.NewInt(int64(startindex))) <= 0 {
		fmt.Println(steps[startindex-1] + 1)
		return
	}
	left := kbig.Sub(kbig, big.NewInt(int64(startindex)))
	cycle := endindex - startindex + 1
	_, loopsleft := kbig.DivMod(left, big.NewInt(int64(cycle)), big.NewInt(int64(cycle)))
	index := MaxOf(int(int64(startindex)+loopsleft.Int64()), 0)
	fmt.Println(steps[index] + 1)
}
