...
func main() {
	...
	A := makeInts(N)
	Prange := make(map[int]int)
	Qrange := make(map[int]int)
	Rrange := make(map[int]int)
	Csum := make([]int, N+1)
	for i := 0; i < N; i++ {
		Csum[i+1] = Csum[i] + A[i]
	}
	LocalPrint(Csum)
	f := func(M int) map[int]int {
		res := make(map[int]int)
		for i := 0; i < N; i++ {
			idx := sort.Search(N, func(mid int) bool {
				return Csum[mid]-Csum[i] >= M
			})
			if Csum[idx]-Csum[i] == M {
				res[i] = idx
			}
		}
		return res
