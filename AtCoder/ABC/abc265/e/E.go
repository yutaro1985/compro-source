...
func main() {
	N, M := nextInt(), nextInt()
	var ans mint
	WA := Pair{nextInt(), nextInt()}
	WC := Pair{nextInt(), nextInt()}
	WE := Pair{nextInt(), nextInt()}
	W := make([]Pair, 0)
	W = append(W, WA, WC, WE)
	Obs := make(map[Pair]int)
	for i := 0; i < M; i++ {
		X, Y := nextInt(), nextInt()
		Obs[Pair{X, Y}] = 1
	}
	// 左からWAをした回数、WCをした回数。WEをした回数はNから引いて求める
	DP := make([][301][301]mint, N+1)
	DP[0][0][0] = 1
	for i := 0; i < N; i++ {
		for j := 0; j <= i; j++ {
			for k := 0; k <= i; k++ {
				...
				cur := Pair{j*WA.first + k*WC.first + l*WE.first, j*WA.second + k*WC.second + l*WE.second}
				for m, v := range W {
					next := cur
					next.first += v.first
					next.second += v.second
					if _, e := Obs[next]; e {
						continue
					}
					switch m {
					case 0:
						DP[i+1][j+1][k].addSelf(DP[i][j][k])
					case 1:
						DP[i+1][j][k+1].addSelf(DP[i][j][k])
					case 2:
						DP[i+1][j][k].addSelf(DP[i][j][k])
					}
				}
			}
		}
