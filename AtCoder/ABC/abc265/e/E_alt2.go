...
func main() {
	...
	DP[0][0] = 1
	now := func(j, k, l int) Pair {
		return Pair{j*WA.first + k*WC.first + l*WE.first, j*WA.second + k*WC.second + l*WE.second}
	}
	for i := 0; i < N; i++ {
		tmp := make([][]mint, N+1)
		for j := 0; j <= N; j++ {
		...
		for j := 0; j <= i; j++ {
			for k := 0; j+k <= i; k++ {
				...
				for m, v := range W {
					...
					switch m {
					case 0:
						tmp[j+1][k].addSelf(DP[j][k])
					case 1:
						tmp[j][k+1].addSelf(DP[j][k])
					case 2:
						tmp[j][k].addSelf(DP[j][k])
					}
				}
			}
		}
