...
func main() {
	N, M := nextInt(), nextInt()
	acquaintances := make([]map[int]bool, N)
	for i := 0; i < N; i++ {
		acquaintances[i] = make(map[int]bool)
	}
	for i := 0; i < M; i++ {
		x, y := nextInt()-1, nextInt()-1
		acquaintances[x][y] = true
		acquaintances[y][x] = true
	}
	var ans int
	for bit := 1; bit < 1<<N; bit++ {
		...
		for i := 0; i < N; i++ {
			if bit>>i&1 == 1 {
				faction = append(faction, i)
			}
		}
		check := true
		for j := 0; j < len(faction); j++ {
			for k := j + 1; k < len(faction); k++ {
				if _, e := acquaintances[faction[j]][faction[k]]; !e {
					check = false
					break
				}
			}
