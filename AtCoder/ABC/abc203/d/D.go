...
func main() {
	...
	Csum2D := init2DInts(N+1, N+1, 0)
	ans := sort.Search(1e9+7, func(mid int) bool {
		// https://qiita.com/drken/items/56a6b68edef8fc605821#4-%E4%BA%8C%E6%AC%A1%E5%85%83%E7%B4%AF%E7%A9%8D%E5%92%8C
		// 前計算
		for i := 0; i < N; i++ {
			for j := 0; j < N; j++ {
				Csum2D[i+1][j+1] = Csum2D[i+1][j] + Csum2D[i][j+1] - Csum2D[i][j]
				if A[i][j] > mid {
					Csum2D[i+1][j+1]++
				}
			}
		...
		for i := 0; i <= N-K; i++ {
			for j := 0; j <= N-K; j++ {
				// K*K区間の累積和
				// midよりも大きい値がK+K/2+1個よりも少なくなる境界値を探す
				if Csum2D[i+K][j+K]+Csum2D[i][j]-Csum2D[i+K][j]-Csum2D[i][j+K] < K*K/2+1 {
					return true
				}
			}
		}
