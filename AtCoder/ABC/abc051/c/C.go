...
func main() {
	...
	dy := ty - sy
	var ans string
	ans += strings.Repeat("R", dx) + strings.Repeat("U", dy)
	ans += strings.Repeat("L", dx) + strings.Repeat("D", dy)
	ans += "D" + strings.Repeat("R", dx+1) + strings.Repeat("U", dy+1) + "L"
	ans += "U" + strings.Repeat("L", dx+1) + strings.Repeat("D", dy+1) + "R"
	fmt.Println(ans)
}
