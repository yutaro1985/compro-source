...
// 転倒数をBITを使って求めるケースO(NlogN)
// https://scrapbox.io/pocala-kyopro/%E8%BB%A2%E5%80%92%E6%95%B0

func main() {
	S := nextLine()
	var ans int
	S = strings.ReplaceAll(S, "a", "0")
	S = strings.ReplaceAll(S, "t", "1")
	S = strings.ReplaceAll(S, "c", "2")
	S = strings.ReplaceAll(S, "o", "3")
	S = strings.ReplaceAll(S, "d", "4")
	S = strings.ReplaceAll(S, "e", "5")
	S = strings.ReplaceAll(S, "r", "6")
	BIT := newBIT(len(S))
	for i := 0; i < 7; i++ {
		n := int(S[i] - '0')
		ans += i - BIT.sum(n)
