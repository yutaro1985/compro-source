...
func main() {
	...
	for i := 0; i < M; i++ {
		...
		a[i] = list.New()
		for j := 0; j < k; j++ {
			a[i].PushBack(nextInt() - 1)
		}
		p := a[i].Front().Value.(int)
		front[p] = append(front[p], i)
		if len(front[p]) == 2 {
			q.PushBack(p)
		}
	}
	for q.Len() > 0 {
		n := q.Front().Value.(int)
		q.Remove(q.Front())
		for _, v := range front[n] {
			a[v].Remove(a[v].Front())
			if a[v].Len() > 0 {
				p := a[v].Front().Value.(int)
				front[p] = append(front[p], v)
				if len(front[p]) == 2 {
					q.PushBack(p)
				}
			}
