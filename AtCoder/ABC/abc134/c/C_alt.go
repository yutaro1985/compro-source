...
// 「累積MAX」という考え方
// https://qiita.com/takuya000885/items/8881ba05d8e6475a1759#abc-037-c--%E7%B7%8F%E5%92%8C
...
func main() {
	...
	right := make([]int, N)
	for i := 1; i < N; i++ {
		left[i] = MaxOf(left[i-1], An[i-1])
		right[N-1-i] = MaxOf(right[N-i], An[N-i])
	}
	for i := 0; i < N; i++ {
		fmt.Println(MaxOf(right[i], left[i]))
	}
}
