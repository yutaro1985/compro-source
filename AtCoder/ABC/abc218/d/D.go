...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		P1[P[i].x] = append(P1[P[i].x], P[i].y)
		P2[P[i].y] = append(P2[P[i].y], P[i].x)
		check[P[i]] = i
	}
	ans := make(map[Tuple]bool)
	for i := 0; i < N; i++ {
		for j := i + 1; j < N; j++ {
			x1, y1 := P[i].x, P[i].y
			x2, y2 := P[j].x, P[j].y
			...
			if k, e := check[Point{x1, y2}]; e {
				if l, e := check[Point{x2, y1}]; e {
					set := make([]int, 4)
					set[0] = i
					set[1] = j
					set[2] = k
					set[3] = l
					sort.Ints(set)
					ans[Tuple{set[0], set[1], set[2], set[3]}] = true
				}
			}
		}
	}
