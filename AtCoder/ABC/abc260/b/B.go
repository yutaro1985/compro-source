...
func main() {
	...
	for i := 0; i < X; i++ {
		ans = append(ans, A[i].idx)
		qualified[A[i].idx-1] = true
	}
	...
	for i, cnt := 0, 0; cnt < Y; i++ {
		if !qualified[B[i].idx-1] {
			ans = append(ans, B[i].idx)
			qualified[B[i].idx-1] = true
			cnt++
		}
	}
	...
	for i, cnt := 0, 0; cnt < Z; i++ {
		if !qualified[S[i].idx-1] {
			ans = append(ans, S[i].idx)
			cnt++
		}
	}
	sort.Ints(ans)
