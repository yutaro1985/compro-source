...
func main() {
	H, W := nextInt(), nextInt()
	S := makeStrings(H)
	Vert := init2DInts(H, W, 0)
	Hori := init2DInts(H, W, 0)
	for i := 0; i < H; i++ {
		if S[i][0] == '.' {
			Hori[i][0]++
		}
	...
	for i := 0; i < H; i++ {
		for j := W - 1; j-1 >= 0; j-- {
			if Hori[i][j] > 0 && Hori[i][j-1] > 0 {
				ChmaxInt(&Hori[i][j-1], Hori[i][j])
			}
		}
	}
	for i := 0; i < W; i++ {
	...
	for i := 0; i < W; i++ {
		for j := H - 1; j-1 >= 0; j-- {
			if Vert[j][i] > 0 && Vert[j-1][i] > 0 {
				ChmaxInt(&Vert[j-1][i], Vert[j][i])
			}
		}
	}
	...
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			...
			ChmaxInt(&ans, Hori[i][j]+Vert[i][j]-1)
		}
	}
	fmt.Println(ans)
}
