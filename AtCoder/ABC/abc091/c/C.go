...
func main() {
	...
	blue := make([]Pair, N)
	for i := 0; i < N; i++ {
		red[i] = Pair{nextInt(), nextInt()}
	}
	for i := 0; i < N; i++ {
		blue[i] = Pair{nextInt(), nextInt()}
	}
	sort.Slice(red, func(i, j int) bool {
		if red[i].a == red[j].a {
			return red[i].b > red[j].b
		} else {
			return red[i].a < red[j].a
		}
	})
	sort.Slice(blue, func(i, j int) bool {
		if blue[i].a == blue[j].a {
			return blue[i].b < blue[j].b
		} else {
			return blue[i].a < blue[j].a
		}
	})
	ans := make([]Pair, 0)
	paired := make([]bool, N)
	for i := 0; i < N; i++ {
		...
		for j := 0; j < N; j++ {
			...
			if max <= red[j].b && blue[i].b > red[j].b {
				max = red[j].b
				idx = j
			}
		}
		if idx >= 0 {
			ans = append(ans, Pair{i, idx})
			paired[idx] = true
			// LocalPrint(i, idx)
		}
	}
