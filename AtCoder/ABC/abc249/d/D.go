...
func main() {
	...
	Aunique := make([]int, 0)
	for k := range check {
		Aunique = append(Aunique, k)
	}
	ReverseSort(sort.IntSlice(Aunique))
	var ans int
	N = len(Aunique)
	for i := 0; i < N; i++ {
		divisors := make([]int, 0)
		for j := 1; j*j <= Aunique[i]; j++ {
			if Aunique[i]%j == 0 {
				divisors = append(divisors, j)
				if Aunique[i]/j != j {
					divisors = append(divisors, Aunique[i]/j)
				}
			}
		}
		sort.Ints(divisors)
		for _, v := range divisors {
			...
			if b == 1 {
				// a == cと同値
				ans += nums[a] * nums[b] * nums[a]
			} else if a == b {
				// c =1 以外ない
				ans += nums[a] * nums[b] * nums[c]
			} else if b == c {
				ans += nums[a] * nums[b] * nums[b]
			} else {
				ans += nums[a] * nums[b] * nums[c]
			}
		}
