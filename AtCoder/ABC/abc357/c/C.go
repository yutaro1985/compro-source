...
func drawCarpet(N int) {
	size := Pow(3, N)
	carpet := make([][]byte, size)
	for i := range carpet {
		carpet[i] = make([]byte, size)
		for j := range carpet[i] {
			carpet[i][j] = '#'
		}
	}
	fillCarpet(carpet, 0, 0, size)
	for i := 0; i < size; i++ {
		fmt.Println(strings.Trim(fmt.Sprint(string(carpet[i])), "[]"))
	}
}

func fillCarpet(carpet [][]byte, x, y, size int) {
	if size == 1 {
		return
	}
	newSize := size / 3
	for i := 0; i < 3; i++ {
		for j := 0; j < 3; j++ {
			if i == 1 && j == 1 {
				for dx := 0; dx < newSize; dx++ {
					for dy := 0; dy < newSize; dy++ {
						carpet[x+dx+newSize][y+dy+newSize] = '.'
					}
				}
			} else {
				fillCarpet(carpet, x+i*newSize, y+j*newSize, newSize)
			}
		}
	}
}
...
//		for i := 0; i < N; i++ {
//			for j := 0; j < N; j++ {
//				fmt.Printf("%s", string(Carpet[i][j]))
//			}
//			fmt.Println()
//		}
//	}
