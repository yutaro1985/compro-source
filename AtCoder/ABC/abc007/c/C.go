...
func main() {
	...
	R, C := nextInt(), nextInt()
	Start := Position{nextInt() - 1, nextInt() - 1}
	Goal := Position{nextInt() - 1, nextInt() - 1}
	c := make([]string, R)
	dist := make([][]int, R)
	for i := 0; i < R; i++ {
		c[i] = nextLine()
	...
	for queue.Len() != 0 {
		...
		for _, v := range d {
			nextP := Position{cur.H + v[0], cur.W + v[1]}
			if nextP.H < 0 || nextP.H >= R || nextP.W < 0 || nextP.W >= C {
				continue
			}
			if c[nextP.H][nextP.W] == '#' {
				continue
			}
			if dist[nextP.H][nextP.W] == -1 {
				dist[nextP.H][nextP.W] = dist[cur.H][cur.W] + 1
				queue.PushBack(nextP)
			}
		}
	}
	fmt.Println(dist[Goal.H][Goal.W])
}
