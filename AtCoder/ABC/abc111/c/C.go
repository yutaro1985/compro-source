...
func main() {
	...
	v1 := make([]int, N/2)
	v2 := make([]int, N/2)
	check1 := make(map[int]int)
	check2 := make(map[int]int)
	for i := 0; i < N; i++ {
		if i%2 == 0 {
			v1[i/2] = nextInt()
			check1[v1[i/2]]++
	...
	nums1 := make([]Pair, 0)
	nums2 := make([]Pair, 0)
	for k, n := range check1 {
		nums1 = append(nums1, Pair{k, n})
	}
	for k, n := range check2 {
		nums2 = append(nums2, Pair{k, n})
	}
	...
	var ans int
	if e1 == o1 {
		ans = N - MaxOf(check1[e1]+check2[o2], check1[e2]+check2[o1])
	} else {
		var n1, n2 int
		n1 = check1[e1]
		n2 = check2[o1]
