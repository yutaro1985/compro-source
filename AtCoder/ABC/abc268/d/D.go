...
func main() {
	...
	ans := make([]string, 0)
	var dfs func(tmp string, cur, rest int, T []string)
	dfs = func(tmp string, cur, rest int, T []string) {
		if rest < 0 {
			return
		}
		if cur == N {
		...
		if len(tmp) >= 1 && tmp[len(tmp)-1] != '_' {
			dfs(tmp+"_", cur, rest-1, T)
		} else {
			dfs(tmp+T[cur], cur+1, rest, T)
			if len(tmp) >= 1 {
				dfs(tmp+"_", cur, rest-1, T)
			}
		}
