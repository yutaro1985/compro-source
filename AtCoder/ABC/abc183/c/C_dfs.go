...
func main() {
	...
	var dist, last, visited int
	visited = 1 << 0
	fmt.Println(dfs(N, K, T, dist, last, visited))
}

func dfs(N, K int, T [][]int, dist, last, visited int) int {
	if visited == (1<<N)-1 {
		dist += T[last][0]
		if dist == K {
			return 1
	...
	for i := 1; i < N; i++ {
		...
		res += dfs(N, K, T, dist+T[last][i], i, visited|(1<<i))
	}
	return res
}
