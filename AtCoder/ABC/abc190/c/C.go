...
func main() {
	N, M := nextInt(), nextInt()
	Conditions := make([]Cond, M)
	for i := 0; i < M; i++ {
		Conditions[i] = Cond{nextInt() - 1, nextInt() - 1}
	}
	K := nextInt()
	Balls := make([]Ball, K)
	for i := 0; i < K; i++ {
		Balls[i] = Ball{nextInt() - 1, nextInt() - 1}
	}
	var ans int
	for bit := 0; bit < 1<<K; bit++ {
		...
		for i := 0; i < K; i++ {
			if bit>>i&1 == 0 {
				Plates[Balls[i].C] = true
			} else {
				Plates[Balls[i].D] = true
			}
		}
		for i := 0; i < M; i++ {
			if Plates[Conditions[i].A] && Plates[Conditions[i].B] {
				cnt++
			}
		}
		ChmaxInt(&ans, cnt)
