...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		ChminInt(&min, s[i])
		slog[i] = math.Log2(float64(s[i]))
		Csumslog[i+1] = Csumslog[i] + slog[i]
	}
	if min > K {
		fmt.Println(0)
		return
	...
	LocalPrint(slog)
	LocalPrint(Csumslog)
	Klog := math.Log2(float64(K))
	// Kbig := big.NewFloat(Klog)
	for i := 0; i < N; i++ {
		ok, ng := i, N
		for Abs(ok-ng) > 1 {
			mid := (ok + ng) / 2
			sum := Csumslog[mid] - Csumslog[i]
			// sumb := big.NewFloat(sum)
			// sum <= K
			// if Kbig.Cmp(sumb) >= 0 || math.Abs(Klog-sum) < eps {
			// 	ok = mid
			// } else {
			// 	ng = mid
			// }
			// LocalPrint(Klog, sum, Klog-sum)
			// if Klog-sum < eps {
