...
func main() {
	...
	for i := 0; i < K; i++ {
		...
		blocklist[C] = append(blocklist[C], D)
		blocklist[D] = append(blocklist[D], C)
	}
	apfriends := make([]int, N)
	for i := 0; i < N; i++ {
		ans := uf.size(i) - 1 - len(friends[i])
		for _, v := range blocklist[i] {
			if uf.same(i, v) {
				ans--
			}
		}
	...
	fmt.Println(strings.Trim(fmt.Sprint(apfriends), "[]"))
}
