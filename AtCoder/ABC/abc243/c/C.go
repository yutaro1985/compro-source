...
func main() {
	...
	P := make([]Axis, N)
	for i := 0; i < N; i++ {
		P[i] = Axis{nextInt(), nextInt(), 'A'}
	}
	S := nextLine()
	for i := 0; i < len(S); i++ {
		P[i].S = S[i]
	}
	check := make(map[int][]Pair)
	for i := 0; i < N; i++ {
		check[P[i].Y] = append(check[P[i].Y], Pair{P[i].X, P[i].S})
	}
	for k := range check {
		sort.Slice(check[k], func(i, j int) bool {
			return check[k][i].X < check[k][j].X
		})
	}
	for _, v := range check {
		tmp := make([]byte, 0)
		for _, s := range v {
			tmp = append(tmp, s.S)
			if len(tmp) >= 2 && string(tmp[len(tmp)-2:]) == "RL" {
				fmt.Println("Yes")
				return
			}
		}
