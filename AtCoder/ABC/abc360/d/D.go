...
func main() {
	...
	for i := 0; i < N; i++ {
		if S[i] == '0' {
			Lefts = append(Lefts, X[i])
		} else {
			Rights = append(Rights, X[i])
		}
	}
	sort.Ints(Lefts)
	sort.Ints(Rights)
	LocalPrint(Rights, Lefts)
	var ans int
	for i := 0; i < len(Rights); i++ {
		start := sort.Search(len(Lefts), func(j int) bool { return Rights[i] < Lefts[j] })
		end := sort.Search(len(Lefts), func(j int) bool { return Rights[i]+T*2 < Lefts[j] })
		LocalPrint(start, end)
		ans += end - start
	}
	fmt.Println(ans)
}
