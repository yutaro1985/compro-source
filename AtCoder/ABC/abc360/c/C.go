...
type Baggeage struct {
	Location, Weight int
}

func main() {
	N := nextInt()
	Baggeages := make([]Baggeage, N)
	A := makeInts(N)
	W := makeInts(N)
	for i := 0; i < N; i++ {
		Baggeages[i] = Baggeage{A[i], W[i]}
	}
	sort.Slice(Baggeages, func(i, j int) bool {
		if Baggeages[i].Location == Baggeages[j].Location {
			return Baggeages[i].Weight < Baggeages[j].Weight
		} else {
			return Baggeages[i].Location < Baggeages[j].Location
		}
	})
	var ans int
	for i := 0; i < N-1; i++ {
		if Baggeages[i].Location == Baggeages[i+1].Location {
			ans += Baggeages[i].Weight
		}
	}
	fmt.Println(ans)
}
