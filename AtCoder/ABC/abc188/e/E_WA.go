...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		queue.PushBack(i)
		dist[i] = Pair{A[i], A[i]}
		group = append(group, []int{})
		var min int
		min = A[i]
		// group[len(group)-1] = append(group[len(group)-1], i)
		for queue.Len() != 0 {
			cur := queue.Front().Value.(int)
			queue.Remove(queue.Front())
			group[len(group)-1] = append(group[len(group)-1], cur)
			for _, v := range G[cur] {
				dist[v] = Pair{MaxOf(dist[cur].Max, A[v]), MinOf(dist[cur].Min, A[v])}
				queue.PushBack(v)
				seen[v] = true
				// ChminInt(&min, A[v])
				ChmaxInt(&ans, A[v]-min)
				// group[len(group)-1] = append(group[len(group)-1], v)
			}
			// if len(G[cur]) == 0 {
			// 	ChmaxInt(&ans, dist[cur].Max-dist[cur].Min)
			// }
		}
	}
	// fmt.Println(group)
