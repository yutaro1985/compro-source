...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		if v, e := check[plans[i].A]; e {
			event[v].B += plans[i].C
		} else {
			event = append(event, Pair{plans[i].A, plans[i].C})
			check[plans[i].A] = len(event) - 1
		}
		if v, e := check[plans[i].B]; e {
			event[v].B -= plans[i].C
		} else {
			event = append(event, Pair{plans[i].B, -plans[i].C})
			check[plans[i].B] = len(event) - 1
		}
	}
	sort.Slice(event, func(i, j int) bool {
		return event[i].A < event[j].A
	})
	cur := event[0].B
	for i := 1; i < len(event); i++ {
		ans += MinOf(cur, C) * (event[i].A - event[i-1].A)
		cur += event[i].B
	}
	fmt.Println(ans)
}
