...
func main() {
	N := nextInt()
	A := makeInts(1 << N)
	tournament := make([]int, 1<<N)
	copy(tournament, A)
	for len(tournament) > 2 {
		tmp := make([]int, 0)
		for i := 0; i+1 < len(tournament); i += 2 {
			tmp = append(tmp, MaxOf(tournament[i], tournament[i+1]))
		}
		tournament = make([]int, len(tmp))
		copy(tournament, tmp)
	}
	ans := MinOf(tournament[0], tournament[1])
	for i := 0; i < len(A); i++ {
		if A[i] == ans {
			fmt.Println(i + 1)
			return
