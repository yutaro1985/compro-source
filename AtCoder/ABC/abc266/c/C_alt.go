...
func main() {
	A := axis{nextInt(), nextInt()}
	B := axis{nextInt(), nextInt()}
	C := axis{nextInt(), nextInt()}
	D := axis{nextInt(), nextInt()}
	vector := func(start, end axis) axis {
		return axis{end.X - start.X, end.Y - start.Y}
	}
	CrossProduct := func(vi, vj axis) int {
		a1 := vi.X
		a2 := vi.Y
		b1 := vj.X
		b2 := vj.Y
	...
	isOK := func(A, B, C, D axis) (res bool) {
		AB := vector(A, B)
		AD := vector(A, D)
		BA := vector(A, B)
		BC := vector(B, C)
		...
		DA := vector(D, A)
		DC := vector(D, C)
		// 外積が0より大きければ角度は180度以下
		res = CrossProduct(AB, AD) > 0 && CrossProduct(BA, BC) > 0 && CrossProduct(CD, CB) > 0 && CrossProduct(DA, DC) > 0
		return res
	}
	if isOK(A, B, C, D) {
		fmt.Println("Yes")
