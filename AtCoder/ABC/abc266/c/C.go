...
func main() {
	A, B, C, D := nextComplex128(), nextComplex128(), nextComplex128(), nextComplex128()
	AB := B - A
	AD := D - A
	BC := C - B
	BA := A - B
	...
	DA := A - D
	DC := C - D
	LocalPrint(imag(cmplx.Conj(AB)*AD), imag(cmplx.Conj(BC)*BA), imag(cmplx.Conj(CD)*CB), imag(cmplx.Conj(DA)*DC))
	if imag(cmplx.Conj(AB)*AD) <= 0 {
		fmt.Println("No")
		return
	}
	if imag(cmplx.Conj(BC)*BA) <= 0 {
		fmt.Println("No")
		return
	}
	if imag(cmplx.Conj(CD)*CB) <= 0 {
		fmt.Println("No")
		return
	}
	if imag(cmplx.Conj(DA)*DC) <= 0 {
		fmt.Println("No")
		return
	}
	fmt.Println("Yes")
