...
func main() {
	...
	C := make([]Circle, N)
	for i := 0; i < N; i++ {
		C[i] = Circle{nextInt(), nextInt(), nextInt()}
	}
	// 円が交わるまたは接する場合、2点を連結とする
	// https://www.try-it.jp/chapters-6533/sections-6587/lessons-6636/
	for i := 0; i < N; i++ {
		for j := i + 1; j < N; j++ {
			x1, y1, r1 := C[i].x, C[i].y, C[i].r
			x2, y2, r2 := C[j].x, C[j].y, C[j].r
			d := Pow(x2-x1, 2) + Pow(y2-y1, 2)
			if Pow(r2-r1, 2) <= d && d <= Pow(r2+r1, 2) {
				uf.unite(i, j)
			}
		}
	}
