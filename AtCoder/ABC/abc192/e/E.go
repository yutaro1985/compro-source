...
func main() {
	N, M, X, Y := nextInt(), nextInt(), nextInt()-1, nextInt()-1
	G := make([][]Train, N)
	for i := 0; i < M; i++ {
		A, B, T, K := nextInt()-1, nextInt()-1, nextInt(), nextInt()
		G[A] = append(G[A], Train{B, T, K})
		G[B] = append(G[B], Train{A, T, K})
	}
	dist := initInts(N, INF)
	pq := make(PQ, 0)
	heap.Init(&pq)
	...
	heap.Push(&pq, &Item{X, 0, 0})
	for pq.Len() != 0 {
		r := heap.Pop(&pq).(*Item)
		if dist[r.to] != r.priority {
			continue
		}
		for _, v := range G[r.to] {
			cost := (v.K-dist[r.to]%v.K)%v.K + v.T
			push(v.to, r.priority+cost)
		}
	}
	ans := dist[Y]
	if ans == INF {
