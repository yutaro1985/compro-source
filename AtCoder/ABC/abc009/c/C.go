...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		T := make([]string, N-i-1)
		notused := make([]string, 0)
		rest := Ssort[i+1:]
		check := make([]bool, N-i-1)

		for j := 0; j < len(rest); j++ {
			for k := 0; k < len(rest); k++ {
				// fmt.Println(rest[k], string(S[i+k+1]))
				if rest[j][0] == S[i+k+1] {
					if !check[k] {
						T[k] = rest[j]
						check[k] = true
				...
				if k == len(rest)-1 {
					notused = append(notused, rest[j])
				}
				// fmt.Println("notused", notused)
				// fmt.Println(T)
			}
		}

		if cnt+len(notused) > K {
			break
		} else {
			for _, v := range notused {
				for j := 0; j < len(T); j++ {
					if T[j] == "" {
						T[j] = v
						break
		...
		// fmt.Println(base, T)
		ans = strings.Join(base, "") + strings.Join(T, "")
		// fmt.Println(ans)
	}
	fmt.Println(ans)
}
