...
// nCk= n*(n-1)*…*(n-k+2)*(n-k+1)/(k*(k-1)*…*2*1)ということを利用するとO(K)で計算できてしまう。

func main() {
	n, a, b := nextInt(), nextInt(), nextInt()
	ans := mint(2).pow(mint(n)).sub(mint(1))
	ans = ans.sub(calcnCk(n, a)).sub(calcnCk(n, b))
	fmt.Println(ans)
}

func calcnCk(N, K int) mint {
	res := mint(1)
	for i := 0; i < K; i++ {
