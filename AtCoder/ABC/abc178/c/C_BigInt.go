...
func main() {
	N := nextInt()
	ans := big.NewInt(1)
	notexists := big.NewInt(1)
	notexists09 := big.NewInt(1)
	big10 := big.NewInt(10)
	big9 := big.NewInt(9)
	big8 := big.NewInt(8)
	for i := 1; i <= N; i++ {
		...
		notexists09.Mul(notexists09, big9)
		notexists09.Mod(notexists09, big.NewInt(int64(mod)))
		notexists.Mul(notexists, big8)
		notexists.Mod(notexists, big.NewInt(int64(mod)))
	}
	ans.Sub(ans, notexists09)
	ans.Sub(ans, notexists09)
	ans.Add(ans, notexists)
	fmt.Println(ans.Mod(ans, big.NewInt(int64(mod))))
}
