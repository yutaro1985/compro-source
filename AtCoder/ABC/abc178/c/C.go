...
func main() {
	...
	for i := 0; i <= N-1; i++ {
		notexists09 += (memo[i] * COM(fac, finv, inv, N, i)) % mod
		notexists09 %= mod
	}
	notexists09 *= 2
	notexists09 %= mod
	ans = MaxOf(ans-notexists, ans-notexists+mod)
	ans = MaxOf(ans-notexists09, ans-notexists09+mod)
	fmt.Println(ans % mod)
}

func COMinit(fac, finv, inv []int, max int) {
	fac[0], fac[1] = 1, 1
	finv[0], finv[1] = 1, 1
	inv[1] = 1
	for i := 2; i < max; i++ {
...
func COM(fac, finv, inv []int, n, k int) int {
	if n < k {
		return 0
	}
	if n < 0 || k < 0 {
