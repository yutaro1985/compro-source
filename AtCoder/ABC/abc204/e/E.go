...
func main() {
	...
	for i := 0; i < M; i++ {
		...
		G[A] = append(G[A], Road{B, C, D})
		G[B] = append(G[B], Road{A, C, D})
	}
	dist := initInts(N, INF)
	dist[0] = 0
	pq := make(PQ, 0)
	...
	for pq.Len() > 0 {
		...
		for _, v := range G[r.to] {
			var t int
			tmp := sort.Search(v.D-1, func(mid int) bool {
				return v.D < (mid+1)*(mid+2)
			})
			if r.priority >= tmp {
				t = r.priority
			} else {
