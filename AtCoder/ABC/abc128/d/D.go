...
func main() {
	...
	var ans, lscore int
	lminus := make([]int, 0)
	for l := 0; l <= MinOf(N, K); l++ {
		if l > 0 {
			if V[l-1] < 0 {
				lminus = append(lminus, V[l-1])
			}
			lscore += V[l-1]
		}
		minus := make([]int, len(lminus))
		sort.Ints(lminus)
		copy(minus, lminus)
		score := lscore
		for i := 0; i < MinOf(K-l, len(minus)); i++ {
			score -= minus[i]
		}
		ChmaxInt(&ans, score)
		var sum int
		for r := 0; r+l < MinOf(N, K); r++ {
			if V[N-r-1] < 0 {
				minus = append(minus, V[N-r-1])
			}
			sum += V[N-r-1]
			score = lscore + sum
			sort.Ints(minus)
			for i := 0; i < MinOf(K-(l+r+1), len(minus)); i++ {
				score -= minus[i]
			}
			ChmaxInt(&ans, score)
		}
