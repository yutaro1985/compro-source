...
func main() {
	...
	var ans int
	for i := 0; i < N; i++ {
		S[i] = Sushi{nextInt(), nextInt()}
		Csum[i+1] = Csum[i] + S[i].v
		ChmaxInt(&ans, Csum[i+1]-S[i].x)
		maxv[i+1] = MaxOf(maxv[i], Csum[i+1]-S[i].x)
	}
	maxvrev := make([]int, N+1)
	for i := N - 1; i >= 0; i-- {
		ChmaxInt(&ans, Csum[N]-Csum[i]-(C-S[i].x))
		maxvrev[N-i] = MaxOf(maxvrev[N-i-1], Csum[N]-Csum[i]-(C-S[i].x))
	}
	for i := 0; i+1 < N; i++ {
		ChmaxInt(&ans, Csum[i+1]-Csum[0]-S[i].x*2+maxvrev[N-i-1])
	}
	for i := N - 1; i > 0; i-- {
		ChmaxInt(&ans, Csum[N]-Csum[i]-(C-S[i].x)*2+maxv[i])
	}
	fmt.Println(ans)
}
