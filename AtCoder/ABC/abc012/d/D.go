...
func main() {
	N, M := nextInt(), nextInt()
	basstops := make([][]RideTime, N)
	for i := 0; i < M; i++ {
		a, b, t := nextInt()-1, nextInt()-1, nextInt()
		basstops[a] = append(basstops[a], RideTime{b, t})
		basstops[b] = append(basstops[b], RideTime{a, t})
	}
	...
	for i := 0; i < N; i++ {
		...
		for queue.Len() != 0 {
			...
			for _, b := range basstops[p] {
				if !seen[b.dist] || dist[b.dist] > dist[p]+b.duration {
					dist[b.dist] = dist[p] + b.duration
					queue.PushBack(b.dist)
					seen[b.dist] = true
				}
			}
		}
