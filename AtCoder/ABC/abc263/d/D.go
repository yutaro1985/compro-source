...
func main() {
	...
	ans := MinOf(MinOf(L, R)*N, Csum[N])
	// x+y>Nとなるケースでは全てLまたは全てRとなる場合だけ考えればよいので、以下x+y<=Nのケースのみ考慮
	// x <= N-y
	// xが0~N-(y+1)の場合の全項の和はL*x+Csum[N-y]-Csum[x]+R*y=L*x-Csum[x]+Csum[N-y]+R*yで求まる
	// 普通にやると二重ループでTLE。ただ、x <= N-yかつx<yとして、任意のxのときのR*y+Csum[N-y]の最小値が予めわかっていればO(N)で行けそう
	//
	Left := make([]int, N+1)
	for x := 0; x <= N; x++ {
		Left[x] = L*x - Csum[x]
