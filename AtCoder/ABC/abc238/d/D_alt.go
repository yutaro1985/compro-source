...
// 実装は簡単だが理屈は解説放送を見ること
// https://youtu.be/Ckuo6w6s_ko
// こちらはkyopro_friendsさんの解説と解説放送の②
// https://atcoder.jp/contests/abc238/editorial/337
// https://twitter.com/kyopro_friends/status/1489958304608518146
// 解説の1つめとやっていることは同じ

func main() {
	T := nextInt()
	solve := func(a, s int) bool {
		t := a * 2
		if t > s {
			return false
		}
		p := a
		q := s - t
		// p := fmt.Sprintf("%064b", a) // AND
		// q := fmt.Sprintf("%064b", s-t) // XOR
		// ANDが1のbitでXORが1にはなり得ないので、そういうbitがあったらNG
		// ⇔ p&q == 0
		for i := 0; ; i++ {
			if (p>>i)&(q>>i) == 1 {
				return false
