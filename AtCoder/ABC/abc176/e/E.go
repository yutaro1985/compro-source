...
// golangにはsetがないので、map+structでそれっぽいことをする
// mapのkeyにはsliceは使えない（arrayなら使える）
// 参考
// http://kitakitabauer.hatenablog.com/entry/2017/04/04/204701
// https://atcoder.jp/contests/abc176/submissions/16132007
...
func main() {
	...
	BombsH := make([]int, W)
	BombsW := make([]int, H)
	var maxBombsH, maxBombsW int
	maxHindex := make([]int, 0)
	maxWindex := make([]int, 0)
	set := make(map[Place]bool)
	for i := 0; i < M; i++ {
		h, w := nextInt()-1, nextInt()-1
		BombsH[w]++
		BombsW[h]++
		set[Place{h, w}] = true
	}
	for _, v := range BombsH {
		maxBombsH = MaxOf(maxBombsH, v)
	}
	for _, v := range BombsW {
		maxBombsW = MaxOf(maxBombsW, v)
	}
	for i, v := range BombsH {
		if v == maxBombsH {
			maxHindex = append(maxHindex, i)
		}
	}
	for i, v := range BombsW {
		if v == maxBombsW {
			maxWindex = append(maxWindex, i)
		}
	}
	ans := maxBombsH + maxBombsW
	for _, mh := range maxHindex {
		for _, mw := range maxWindex {
			p := Place{mw, mh}
			if _, e := set[p]; !e {
				fmt.Println(ans)
				return
