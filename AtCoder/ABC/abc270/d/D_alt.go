...
// 解説放送のやり方
// https://www.youtube.com/watch?v=LOFHSAJqUEE

func main() {
	N, K := nextInt(), nextInt()
	A := makeInts(K)
	// DP[N][0]->N個、高橋くんのターンからゲームを始めたときに高橋くんが取れる数
	// DP[N][1]->N個、青木くんのターンからゲームを始めたときに高橋くんが取れる数
	DP := make([][2]int, N+1)
	for i := 1; i <= N; i++ {
		DP[i][1] = INF
	}
