...
func main() {
	...
	Edgescar := make([][]Edge, N)
	for i := 0; i < N; i++ {
		Edgescar[i] = make([]Edge, N)
		for j := 0; j < N; j++ {
			d := D[i][j] * A
			if i == j {
				d = 0
			}
			Edgescar[i][j] = Edge{j, d}
		}
	}
	Edgestrain := make([][]Edge, N)
	for i := 0; i < N; i++ {
		Edgestrain[i] = make([]Edge, N)
		for j := 0; j < N; j++ {
			d := D[i][j]*B + C
			if i == j {
				d = 0
			}
			Edgestrain[i][j] = Edge{j, d}
		}
	}

	distcar := initInts(N, INF)
	Dijkstra(0, Edgescar, distcar)
	disttrain := initInts(N, INF)
	Dijkstra(N-1, Edgestrain, disttrain)
	ans := INF
	for i := 0; i < N; i++ {
		ChminInt(&ans, distcar[i]+disttrain[i])
	}
	fmt.Println(ans)
}
