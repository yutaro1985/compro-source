...
func main() {
	...
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			if S[i][j] == '#' {
				C[Coordinate{i, j}] = cnt
				cnt++
			}
		}
	}
	uf := newUnionFind(len(C))
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			if S[i][j] == '.' {
				continue
			}
			for k := MaxOf(i-1, 0); k <= MinOf(i+1, H-1); k++ {
				for l := MaxOf(j-1, 0); l <= MinOf(j+1, W-1); l++ {
					if i == k && j == l {
						continue
					}
					if S[k][l] == '#' {
						uf.unite(C[Coordinate{i, j}], C[Coordinate{k, l}])
					}
				}
			}
		}
