...
func main() {
	...
	ans := 0
	uf := newUnionFind(H * W)
	LocalPrint(uf.parent)
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			if S[i][j] == '.' {
				continue
			}
			for k := MaxOf(0, i-1); k <= MinOf(i+1, H-1); k++ {
				for l := MaxOf(0, j-1); l <= MinOf(j+1, W-1); l++ {
					if S[k][l] == '#' {
						uf.unite(index(i, j, W), index(k, l, W))
					}
				}
			}
		}
	...
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			if S[i][j] == '.' && uf.parent[index(i, j, W)] == -1 {
				ans--
			}
		}
	}
