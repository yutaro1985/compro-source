...
func main() {
	...
	for i := 0; i < N; i++ {
		Coordinates[i] = Coordinate{nextInt(), nextInt()}
	}
	...
	for {
		var sum float64
		for i := 1; i < N; i++ {
			n, m := permutation[i], permutation[i-1]
			sum += math.Sqrt(math.Pow(float64(Coordinates[n].X-Coordinates[m].X), 2) + math.Pow(float64(Coordinates[n].Y-Coordinates[m].Y), 2))
		}
		ans += sum
		if !NextPermutation(sort.IntSlice(permutation)) {
			break
		}
	}
	ans /= float64(factorial(N))
	fmt.Println(ans)
}
