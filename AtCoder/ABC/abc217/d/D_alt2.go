...
// Union Findを使った解法
// 参考
// https://atcoder.jp/contests/abc217/submissions/25585964
...
func main() {
	...
	for i := Q - 1; i >= 0; i-- {
		switch Queries[i].c {
		case 1:
			uf.unite(idxs[Queries[i].x], idxs[Queries[i].x]+1)
		case 2:
			idx := sort.Search(len(x), func(mid int) bool {
				return x[mid] >= Queries[i].x
			}) - 1
			// idx := sort.SearchInts(x, Queries[i].x) - 1
			a := uf.size(idx)
			LocalPrint(idx, uf, a)
			ans = append(ans, a)
		}
