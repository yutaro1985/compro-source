...
func main() {
	...
	// ここまでで、最長の文字数が求まる。
	// 以下で復元する。
	ans := make([]byte, dp[len(s)][len(t)])
	// インデックスのズレに注意
	for i, j, l := len(s)-1, len(t)-1, dp[len(s)][len(t)]-1; l >= 0; {
		if s[i] == t[j] {
			ans[l] = s[i]
			i--
			j--
	...
	// for i, j, l := len(s), len(t), dp[len(s)][len(t)]; l > 0; {
	// 	if s[i-1] == t[j-1] {
	// 		ans[l-1] = s[i-1]
	// 		i--
	// 		j--
	// 		l--
	// 		// i-1の値を参照するのでi >= 1になるようなインデックスでないとエラーになる
