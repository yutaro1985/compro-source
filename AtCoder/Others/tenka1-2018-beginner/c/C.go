...
func main() {
	...
	for i := 0; i < N-1; i++ {
		...
		switch i % 2 {
		case 0:
			n = deque.Back().Value.(int)
			deque.Remove(deque.Back())
		case 1:
			n = deque.Front().Value.(int)
	...
	for i := 1; i < len(memo); i++ {
		tmp += Abs(memo[i] - memo[i-1])
	}
	r := deque.Back().Value.(int)
	deque.Remove(deque.Back())
	ans = tmp + MaxOf(Abs(memo[0]-r), Abs(memo[len(memo)-1]-r))
	for i := 0; i < N; i++ {
		deque.PushBack(A[i])
	}
	memo2 := make([]int, 0)
	for i := 0; i < N-1; i++ {
		...
		memo2 = append(memo2, n)
	}
	tmp = 0
	r = deque.Back().Value.(int)
	for i := 1; i < len(memo2); i++ {
		tmp += Abs(memo2[i] - memo2[i-1])
	}
	ChmaxInt(&ans, tmp+MaxOf(Abs(memo2[0]-r), Abs(memo[len(memo2)-1]-r)))
	fmt.Println(ans)
}
