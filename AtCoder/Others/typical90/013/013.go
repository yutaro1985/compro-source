...
func main() {
	...
	for i := 0; i < M; i++ {
		...
		Roads[A] = append(Roads[A], Road{B, C})
		Roads[B] = append(Roads[B], Road{A, C})
	}
	distFrom1 := initInts(N, INF)
	distFromN := initInts(N, INF)
	pq := make(PQ, 0)
	heap.Init(&pq)
	...
	for pq.Len() > 0 {
		r := heap.Pop(&pq).(*Item)
		if distFrom1[r.to] != r.priority {
			continue
		}
		for _, v := range Roads[r.to] {
			push(v.to, r.priority+v.C, distFrom1)
		}
	}
	push(N-1, 0, distFromN)
	for pq.Len() > 0 {
		r := heap.Pop(&pq).(*Item)
		if distFromN[r.to] != r.priority {
			continue
		}
		for _, v := range Roads[r.to] {
			push(v.to, r.priority+v.C, distFromN)
		}
	}
	for i := 0; i < N; i++ {
