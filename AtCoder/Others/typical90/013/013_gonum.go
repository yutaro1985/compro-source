...
func main() {
	...
	absent := math.MaxFloat64 // the wieght returned for absent edges

	G := simple.NewWeightedUndirectedGraph(self, absent)
	for i := 0; i < N; i++ {
		G.AddNode(simple.Node(int64(i + 1)))
	}
	for i := 0; i < M; i++ {
		A, B, C := int64(nextInt()), int64(nextInt()), nextFloat64()
		WE := simple.WeightedEdge{F: simple.Node(A), T: simple.Node(B), W: C}
		G.SetWeightedEdge(WE)
	}
	distFrom1 := path.DijkstraFrom(simple.Node(1), G)
	distFromN := path.DijkstraFrom(simple.Node(N), G)
	for i := 0; i < N; i++ {
		fmt.Println(int(distFrom1.WeightTo(int64(i+1))) + int(distFromN.WeightTo(int64(i+1))))
	}
}
