...
func main() {
	L, R := mint(nextInt()), mint(nextInt())
	memo := make([]mint, 0)
	memo = append(memo, 0)
	memo = append(memo, 45)
	for i := 2; i <= 18; i++ {
		f := mint(Pow(10, i-1))
		l := mint(Pow(10, i)).sub(1)
		next := memo[i-1]
		next.addSelf(f.add(l).mul(l.sub(f).add(1)).div(2).mul(mint(i)))
		memo = append(memo, next)
	}
	count := func(N mint) mint {
		var res mint
		digits := mint(len(strconv.Itoa(int(N))))
		if digits == 1 {
			return N * (N + 1) / 2
		}
		f := mint(10).pow(digits - 1)
		rest := N.sub(f).add(1)
		res = memo[digits-1]
		res.addSelf(f.add(N).mul(rest).div(2).mul(digits))
		return res
	}
	ans := count(R).sub(count(L - 1))
	fmt.Println(ans)
}
