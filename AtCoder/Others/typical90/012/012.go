...
func main() {
	...
	for ; Q > 0; Q-- {
		...
		case 1:
			...
			red[r][c] = true
			for _, v := range d {
				nv := Position{v.H + r, v.W + c}
				if nv.H < 0 || nv.H >= H || nv.W < 0 || nv.W >= W {
					continue
				}
				if red[nv.H][nv.W] {
					uf.unite(pos(r, c), pos(nv.H, nv.W))
				}
			}
		case 2:
			ra, ca, rb, cb := nextInt()-1, nextInt()-1, nextInt()-1, nextInt()-1
			if red[ra][ca] && red[rb][cb] && uf.same(pos(ra, ca), pos(rb, cb)) {
				fmt.Println("Yes")
			} else {
				fmt.Println("No")
			}
