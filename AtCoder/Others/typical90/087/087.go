...
func main() {
	...
	WarshallFloyd := func(d [][]int) {
		for k := 0; k < N; k++ {
			for i := 0; i < N; i++ {
				for j := 0; j < N; j++ {
					ChminInt(&d[i][j], d[i][k]+d[k][j])
				}
			}
		}
	}
	count := func(X int) int {
		dist := init2DInts(N, N, 0)
		copydist(dist, X)
		WarshallFloyd(dist)
		var cnt int
		for i := 0; i < N; i++ {
	...
	upper := sort.Search(INF, func(X int) bool {
		cnt := count(X)
		return cnt < K
	})
	lower := sort.Search(INF, func(X int) bool {
		cnt := count(X)
		return cnt <= K
	})
	// sort.Seachはすべてtrueの場合0になるが、K >= 1なのでその分を考慮する必要がある。
	if upper == lower {
		fmt.Println(0)
	} else if upper == INF && lower != INF {
		fmt.Println("Infinity")
	} else {
		fmt.Println(upper - MaxOf(1, lower))
	}
}
