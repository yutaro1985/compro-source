...
func main() {
	s := nextLine()
	RSF := func(s string) (string, int) {
		var suit string
		var idxS, idxH, idxD, idxC int
		minidx := INF
		Spade := []string{"S10", "SJ", "SQ", "SK", "SA"}
		Heart := []string{"H10", "HJ", "HQ", "HK", "HA"}
		Dia := []string{"D10", "DJ", "DQ", "DK", "DA"}
		Club := []string{"C10", "CJ", "CQ", "CK", "CA"}
		for _, v := range Spade {
	...
	suit, idx := RSF(s)
	ans := s[:idx]
	ans = strings.ReplaceAll(ans, suit+"10", "")
	ans = strings.ReplaceAll(ans, suit+"A", "")
	ans = strings.ReplaceAll(ans, suit+"J", "")
	ans = strings.ReplaceAll(ans, suit+"Q", "")
	ans = strings.ReplaceAll(ans, suit+"K", "")
	if len(ans) == 0 {
		fmt.Println(0)
	} else {
