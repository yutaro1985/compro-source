...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		if i == 0 {
			...
			Xi = string(X[:i]) + tmp + string(X[i+1:])
		}
		if strings.Count(Xi, "1") == 0 {
			fmt.Println(0)
			continue
		}
		// fmt.Println(Xi)
		num, m := new(big.Int), new(big.Int)
		mod := big.NewInt(int64(strings.Count(Xi, "1")))
		fmt.Sscanf(Xi, "%b", num)
		cnt := 1
		_, m = num.DivMod(num, mod, m)
		numint := int(m.Int64())
		for numint > 0 {
			numint %= bits.OnesCount(uint(numint))
			cnt++
		}
		fmt.Println(cnt)
	}
