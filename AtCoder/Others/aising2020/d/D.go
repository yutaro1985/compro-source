...
func main() {
	...
	for i := N - 1; i >= 0; i-- {
		if X[i] == '1' {
			x10pls += bit1 % (onescount + 1)
			if onescount > 1 {
				x10mns += bit2 % (onescount - 1)
			}
		}
		bitp[i] = bit1 % (onescount + 1)
		bit1 = (bit1 * 2) % (onescount + 1)
		if onescount > 1 {
			bitm[i] = bit2 % (onescount - 1)
			bit2 = (bit2 * 2) % (onescount - 1)
		}
	}
	for i := 0; i < N; i++ {
		var onescounti int
