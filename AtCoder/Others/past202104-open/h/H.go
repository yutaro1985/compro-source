...
func main() {
	N, M, K, Q := nextInt(), nextInt(), nextInt(), nextInt()
	P0 := make([]int, 0)
	P1 := make([]int, 0)
	for i := 0; i < N; i++ {
		P, T := nextInt(), nextInt()
	...
	ans := sum + Ceil(canning, K)*Q
	for i := 0; idx+i < len(P1); i++ {
		if MinOf((len(P0)), M)-1-i < 0 {
			break
		}
		sum -= P0[MinOf((len(P0)), M)-1-i]
		sum += P1[idx+i]
		canning++
		opener = Ceil(canning, K) * Q
		ChminInt(&ans, sum+opener)
	}
	fmt.Println(ans)
}
