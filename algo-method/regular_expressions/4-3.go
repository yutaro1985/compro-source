...
func main() {
	S := nextLine()
	S = strings.ReplaceAll(S, "algomethod", "aaaaa")
	S = strings.ReplaceAll(S, "algorithm", "bbbbb")
	r := regexp.MustCompile("algo[a-z]{5,}")
	check := r.FindStringSubmatch(S)
	var cnt int
	for _, v := range check {
		if strings.HasPrefix(v, "algomethod") || strings.HasPrefix(v, "algorithm") {
			fmt.Println("No")
			return
		}
		cnt++
