...
func main() {
	...
	H := func(S []int, x int) []int {
		...
		for x >= 0 {
			...
			for {
				...
				max := MaxOf(S[k], S[k*2+1])
				if k*2+2 < N {
					ChmaxInt(&max, S[k*2+2])
				}
				var idx int
				if S[k] == max {
					break
	...
	for i := N - 1; i >= 0; i-- {
		...
		A = H(A[:i], 0)
		if i == M {
			fmt.Println(strings.Trim(fmt.Sprint(A), "[]"), strings.Trim(fmt.Sprint(ans[i:]), "[]"))
		}
	}
