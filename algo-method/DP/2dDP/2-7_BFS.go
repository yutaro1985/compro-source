...
func main() {
	...
	for q.Len() > 0 {
		...
		for _, v := range d {
			...
			if dist[nextP.H][nextP.W] > dist[P.H][P.W]+A[nextP.H][nextP.W] {
				dist[nextP.H][nextP.W] = dist[P.H][P.W] + A[nextP.H][nextP.W]
				q.PushBack(nextP)
			}
		}
	}
	fmt.Println(dist[N-1][0])
}

var d = []Position{{1, 0}, {0, -1}}
