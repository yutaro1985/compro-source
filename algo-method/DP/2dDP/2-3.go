...
func main() {
	...
	for i := 1; i < N; i++ {
		DP[i][0] = MaxOf(DP[i-1][1], DP[i-1][2]) + A[i][0]
		DP[i][1] = MaxOf(DP[i-1][0], DP[i-1][2]) + A[i][1]
		DP[i][2] = MaxOf(DP[i-1][0], DP[i-1][1]) + A[i][2]

	}
	fmt.Println(MaxOf(DP[N-1][0], DP[N-1][1], DP[N-1][2]))
}
